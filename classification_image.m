...
for ifile = 1:nfiles
	...
	for elt = 2:size(final_array, 1)
		...
		if final_array(elt, 2) == 1
			...
			vec_resp_right_n(length(vec_resp_right_n) + 1) = -final_array(elt, 1);
		else
			vec_resp_left(length(vec_resp_left) + 1) = -final_array(elt - 1, 1);
			vec_resp_left_n(length(vec_resp_left_n) + 1) = -final_array(elt, 1);
		end
	end
	...
	% we get all the gabors
	for elt = 1:1:length(vec_resp_left_n)
		...
		Image_Array_Left_n{1, elt} = Screen('GetImage', scr.main, pos_rec, 'drawBuffer', [], []);
	
	end
	
	all_arrays_left_n = cat(3, Image_Array_Left_n{:});
	meanMatrixleft_n = mean(all_arrays_left_n, 3);
	
	Image_Array_Right_n = {};

	for elt = 1:length(vec_resp_right_n)

		Screen('DrawTexture', scr.main, gaborid, [],...
		[], vec_resp_right_n(elt), [], [], [],...
		[], kPsychDontDoRotation, [phase, freq,...
		sigma, contrast, aspectRatio, 0, 0, 0]);
			
		Image_Array_Right_n{1, elt} = Screen('GetImage', scr.main, pos_rec, 'drawBuffer', [], []);
	
	end

	all_arrays_right_n = cat(3, Image_Array_Right_n{:});
	meanMatrixright_n = mean(all_arrays_right_n, 3);
	...
	class_image_left_n = uint8((meanMatrixleft_n - meanMatrixright_n) + 191 );
	class_image_right_n = uint8((meanMatrixright_n - meanMatrixleft_n) + 191);
	
		
	r_coherent = corr2(class_image_left, class_image_left_n);
	
	r_corr(1, length(r_corr) + 1) = r_coherent;
	
	r_incoherent = corr2(class_image_left, class_image_right_n);
	
	r_incorr(1, length(r_incorr) + 1) = r_incoherent;
	
	r_coherent = corr2(class_image_right, class_image_right_n);
	
	r_corr(1, length(r_corr) + 1) = r_coherent;
	
	r_incoherent = corr2(class_image_right, class_image_left_n);
	...
	%cell_CI(length(cell_CI) + 1) = {class_image_left};
	%cell_CI(length(cell_CI) + 1) = {class_image_left_n};
	%cell_CI(length(cell_CI) + 1) = {class_image_right};
	%cell_CI(length(cell_CI) + 1) = {class_image_right_n};
	...
	%name_left_CI = sprintf('%s_CI_left_control.jpg', shorter_name);
	%name_right_CI = sprintf('%s_CI_right_control.jpg', shorter_name);
	%imwrite(class_image_left_n, name_left_CI)
	%imwrite(class_image_right_n, name_right_CI)
	
			
	% we then continue to the next subject
	clearvars -except csv_files nfiles cell_CI r_corr r_incorr
